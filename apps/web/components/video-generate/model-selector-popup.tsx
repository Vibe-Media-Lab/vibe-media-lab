...
interface VideoModelSelectorPopupProps {
  value: string
  onChange: (id: string) => void
  capability: 'image-to-video' | 'text-to-video'
  disabled?: boolean
  triggerClassName?: string
  popoverSide?: 'top' | 'bottom' | 'right'
}

export function VideoModelSelectorPopup({ value, onChange, capability, disabled, triggerClassName, popoverSide }: VideoModelSelectorPopupProps) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        ...
        align={popoverSide === 'right' ? 'start' : 'start'}
        sideOffset={popoverSide === 'right' ? 12 : undefined}
        className={cn(
          'p-0',
          popoverSide === 'right' ? 'w-[340px]' : 'w-80',
        )}
      >
        {/* Search */}
        ...
        <div className={cn(
          'overflow-y-auto p-2',
          popoverSide === 'right' ? 'max-h-[calc(100vh-200px)]' : 'max-h-[320px]',
        )}>
